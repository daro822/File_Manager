...
class Interface():
    ...
    def __init__(self, root, pm):
        ...
        self.pm = pm
        self.frame_top = Frame(self.root,  width=450, height=35, bg='green').grid(columnspan=8, sticky='we')

        self.bttn1 = Button(self.frame_top, width=5, text='+ dir',command=lambda: self.lb_trigger(self.add_dir_trigger))
        self.bttn1.grid(row=0, column=1, sticky='w', padx=5)
        self.bttn2 = Button(self.frame_top, width=5, text='+ file',command=lambda: self.lb_trigger(self.add_file_trigger))
        ...
        self.lb_1 = Listbox(self.frame_middle, width=25)
...
https://realpython.com/intermediate-python-project-ideas/#file-manager
'''

'''
kurs dotyczacy zakładek
